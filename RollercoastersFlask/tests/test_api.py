...
def test_recommender_intense(client):
    response = client.post("/recommender", data={"length": "135"}, follow_redirects=True)
    assert response.status_code == 200
...
# ---------------------------------------------------------
@pytest.mark.parametrize("length,expected_coasters", [
    (110, [b"Alpen_Express", b"Pegasus", b"Ba_a_a_Express"]),
    (125, [b"Wodan", b"Arthur", b"Cancan_Coaster"]),
    (135, [b"Voltron", b"Blue_Fire", b"Euromir"]),
    (150, [b"Silverstar"]),
])
def test_recommender_paths(client, length, expected_coasters):
    response = client.post("/recommender", data={"length": str(length)}, follow_redirects=True)
    assert response.status_code == 200
    assert any(c in response.data for c in expected_coasters)
